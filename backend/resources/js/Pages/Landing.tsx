    ...
    <>
      ...
      <div className="min-h-screen bg-black text-white selection:bg-red-600">
        ...
        <nav className="fixed top-0 w-full z-50 flex justify-between items-center px-6 md:px-12 py-6 bg-gradient-to-b from-black/80 to-transparent">
          ...
          {auth.user ? (
            <Link href={route('dashboard')} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded font-bold transition-colors">
              Dashboard
            </Link>
          ) : (
            <Link href={route('login')} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded font-bold transition-colors">
              Entrar
            </Link>
          )}
        </nav>
        ...
        <div className="relative h-screen flex items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 text-center max-w-4xl px-4">
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-center">
              {auth.user ? (
                <Link href={route('dashboard')} className="bg-red-600 hover:bg-red-700 text-white text-xl px-12 py-4 rounded font-bold transition-all transform hover:scale-105 shadow-[0_0_30px_rgba(220,38,38,0.5)]">
                  IR PARA DASHBOARD
                </Link>
              ) : (
                <Link href={route('register')} className="bg-red-600 hover:bg-red-700 text-white text-xl px-12 py-4 rounded font-bold transition-all transform hover:scale-105 shadow-[0_0_30px_rgba(220,38,38,0.5)]">
                  COMEÇAR AGORA
                </Link>
              )}
            </div>
